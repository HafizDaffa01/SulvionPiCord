...
class Bot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        try:
            ...
            for cmd_app in synced:
                if hasattr(cmd_app, 'id'):
                    print(f" - /{cmd_app.name} (ID: {cmd_app.id})")
                else:
        ...
        for name, func in self.no_prefix_commands.items():
            if content == name or content.startswith(name + " "):
                ...
                try:
                    ...
                    await self.on_command_error(message, e) # type: ignore (message is not ctx but handled by our error handler)
                return
    ...
    def onRecv(self, name: str, *args: Any, **kwargs: Any) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
        ...
        Args:
            ...
            *args: SPC properties like NOPREFIX.
        """
        def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
            ...
            async def wrapper(ctx: Union[commands.Context, Context], *cmd_args: Any, **cmd_kwargs: Any) -> Any:
                ...
                if asyncio.iscoroutinefunction(func):
                    await func(real_ctx, *cmd_args, **cmd_kwargs)
                else:
                    sync_ctx = SyncContext(real_ctx) # type: ignore
                    await asyncio.to_thread(func, sync_ctx, *cmd_args, **cmd_kwargs)
    ...
    async def on_command_error(self, ctx: Union[commands.Context, discord.Interaction, discord.Message], error: Any) -> None:
        ...
        """
        # Convert to SPC Context for easy replying
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            return # Ignore CommandNotFound
        else:
            # For unhandled/generic errors, prepare a traceback for the user but DON't print to terminal
            tb = "".join(traceback.format_exception(type(error), error, error.__traceback__))
            if len(tb) > 1800: tb = tb[:1800] + "..."
            msg = f"❌ **Internal Error:** {error}\n```python\n{tb}\n```"
        
        # Only log to bot console if explicitly requested or for really unexpected errors
        # In this mode, we keep terminal clean as per user request.
        
        try:
            await spc_ctx.reply(msg, hidden=True)
        except:
            # Final fallback if ctx.reply fails (e.g. channel lost)
            pass
